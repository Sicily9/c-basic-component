...
static void update(gp_handler *handler){
	...
	handler->loop->update_handler(handler);
}

void init_gp_handler(gp_handler *handler, gp_loop *loop, int fd)
{
	tmp->loop = loop;
	tmp->fd = fd;
	tmp->_none_event = 0;
	tmp->_read_event = POLLIN | POLLPRI;
	tmp->_write_event = POLLOUT;
	tmp->index = -1;
	tmp->_events = 0;
	tmp->_revents = 0;
